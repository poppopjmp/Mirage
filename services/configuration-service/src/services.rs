use crate::models::{
    ConfigItem, ConfigVersion, ConfigNamespace, ConfigValueType, 
    CreateConfigRequest, UpdateConfigRequest, ConfigResponse, 
    ConfigVersionResponse, ConfigNamespaceResponse, CreateNamespaceRequest,
    PaginatedResponse
};
...
use redis::{Client as RedisClient, Commands, AsyncCommands};
use std::sync::Arc;
use chrono::Utc;
use uuid::Uuid;
...
impl ConfigService {
    ...
    pub async fn create_config(&self, request: CreateConfigRequest, user_id: Option<String>) -> Result<ConfigResponse> {
        // Validate namespace
        let namespace = self.repo.get_namespace(&request.namespace).await?
            .ok_or_else(|| Error::NotFound(format!("Namespace '{}' not found", request.namespace)))?;
        
        // Check if config with same key+namespace already exists
        if self.repo.get_config_by_key(&request.key, &request.namespace).await?.is_some() {
        ...
        if let Some(schema) = &request.schema {
            self.validator.validate_against_schema(&request.value, schema)?;
        }
        
        // Validate value type
        self.validator.validate_value_type(&request.value, &request.value_type)?;
    ...
    pub async fn get_config(&self, config_id: Uuid) -> Result<ConfigResponse> {
        let config = self.repo.get_config_by_id(&config_id).await?
            .ok_or_else(|| Error::NotFound(format!("Configuration with ID '{}' not found", config_id)))?;
    ...
    pub async fn get_config_by_key(&self, key: &str, namespace: &str) -> Result<ConfigResponse> {
        ...
        let config = self.repo.get_config_by_key(key, namespace).await?
    ...
    pub async fn update_config(&self, config_id: Uuid, request: UpdateConfigRequest, user_id: Option<String>) -> Result<ConfigResponse> {
        // Get current config
        let mut config = self.repo.get_config_by_id(&config_id).await?
            .ok_or_else(|| Error::NotFound(format!("Configuration with ID '{}' not found", config_id)))?;
        ...
        // Validate value type
        self.validator.validate_value_type(&request.value, &config.value_type)?;
        ...
        self.audit_service.log_update(
            ...
            user_id.as_deref(),
            &change_details,
            Some(format!("Updated value and incremented version to {}", config.version)),
        ).await?;
    ...
    pub async fn delete_config(&self, config_id: Uuid, user_id: Option<&str>) -> Result<()> {
        ...
        // Remove from cache
        self.invalidate_cache(&config.key, &config.namespace).await?;
    ...
    pub async fn list_configs(
        ...
        let (configs, total) = self.repo.list_configs(
            namespace,
            tag,
            key_contains,
            page,
        ...
        // Map to responses
        let items: Vec<ConfigResponse> = configs.into_iter().map(|config| {
            ConfigResponse {
                id: config.id,
                key: config.key,
                namespace: config.namespace,
    ...
    pub async fn get_config_history(&self, config_id: Uuid) -> Result<Vec<ConfigVersionResponse>> {
        ...
        // Map to responses
        let responses: Vec<ConfigVersionResponse> = versions.into_iter().map(|version| {
    ...
    pub async fn create_namespace(&self, request: CreateNamespaceRequest, user_id: Option<&str>) -> Result<ConfigNamespaceResponse> {
        // Check if namespace exists
        if self.repo.get_namespace(&request.name).await?.is_some() {
            return Err(Error::Conflict(format!("Namespace '{}' already exists", request.name)));
        }
    ...
    // List namespaces
    pub async fn list_namespaces(&self, page: u64, per_page: u64) -> Result<PaginatedResponse<ConfigNamespaceResponse>> {
        ...
        for namespace in namespaces {
            let count = self.repo.count_configs_in_namespace(&namespace.name).await?;
    ...
    pub async fn get_raw_config_value(&self, key: &str, namespace: &str) -> Result<serde_json::Value> {
        let config = self.get_config_by_key(key, namespace).await?;
        Ok(config.value)
    }
    
    // Get strongly-typed configuration with default fallback
    pub async fn get_config_value<T: serde::de::DeserializeOwned>(&self, key: &str, namespace: &str, default: T) -> Result<T> {
        match self.get_config_by_key(key, namespace).await {
            Ok(config) => {
                serde_json::from_value::<T>(config.value)
        ...
        if is_secret {
            match value {
                ...
                serde_json::Value::Object(obj) => {
                    ...
                    for (k, v) in obj {
                        masked_obj.insert(k.clone(), serde_json::Value::String("*****".to_string()));
                    }
    ...
    async fn cache_config(&self, config: &ConfigItem) -> Result<()> {
        ...
        conn.set_ex::<_, _, ()>(&key, value, self.config.redis.cache_ttl_seconds as usize).await
    ...
    async fn get_from_cache(&self, key: &str, namespace: &str) -> Result<Option<ConfigItem>> {
        ...
        if let Some(json) = value {
            let config = serde_json::from_str::<ConfigItem>(&json)
