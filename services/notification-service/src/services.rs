...
use crate::models::{
    Notification, NotificationDelivery, Subscription, SendNotificationRequest, 
    NotificationResponse, NotificationStatus, NotificationStatusResponse, 
    DeliveryStatusResponse, CreateSubscriptionRequest, SubscriptionResponse,
    NotificationChannelRequest
};
use crate::repositories::{DbPool, NotificationRepository};
use crate::channels::{Channel, get_channel};
...
impl NotificationService {
    ...
    pub async fn send_notification(&self, request: SendNotificationRequest) -> Result<NotificationResponse> {
        // Validate request
        if request.channels.is_empty() {
            return Err(Error::Validation("At least one notification channel must be provided".into()));
        }
        
        // Generate subject and content
        let (subject, content) = if let (Some(custom_subject), Some(custom_content)) = (&request.custom_subject, &request.custom_content) {
            ...
            let subject = self.templates.render_subject(&request.notification_type, &serde_json::to_value(&request.data)?)?;
            let content = self.templates.render_content(&request.notification_type, &serde_json::to_value(&request.data)?)?;
            (subject, content)
        } else {
    ...
    pub async fn get_notification_status(&self, notification_id: Uuid) -> Result<NotificationStatusResponse> {
        // Get notification
        let notification = self.repo.get_notification(&notification_id).await?
            .ok_or_else(|| Error::NotFound(format!("Notification with ID {} not found", notification_id)))?;
            
        // Get all deliveries for this notification
        let deliveries = self.repo.get_notification_deliveries(&notification_id).await?;
        
        // Convert to delivery status responses
        let delivery_statuses: Vec<DeliveryStatusResponse> = deliveries.iter().map(|d| {
    ...
    pub async fn create_subscription(&self, request: CreateSubscriptionRequest) -> Result<SubscriptionResponse> {
...
async fn process_pending_deliveries(repo: &NotificationRepository, config: &Arc<AppConfig>) -> Result<usize> {
    // Get pending deliveries
    let deliveries = repo.get_pending_deliveries(config.worker.batch_size as i64).await?;
    ...
    for delivery in deliveries {
        ...
        if let Some(notification) = repo.get_notification(&delivery.notification_id).await? {
            ...
            match channel.send(&delivery, &notification.content, &notification.subject).await {
                Ok(_) => {
                ...
                Err(e) => {
                    ...
                    let retry_delay = config.webhook.retry_delay_seconds * (2_u64.pow(retry_count.min(5) as u32));
                    ...
                    tracing::warn!(
                        ...
                        delivery.id, e, retry_count, config.webhook.max_retries
                    );
                }
            }
        }
