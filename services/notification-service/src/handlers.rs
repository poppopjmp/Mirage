...
use crate::models::{
    SendNotificationRequest, CreateSubscriptionRequest
};
...
async fn send_notification(
    ...
    let result = notification_service.send_notification(request.into_inner()).await
        .map_err(|e| {
...
async fn get_notification_status(
    ...
    let notification_id = Uuid::parse_str(&id).map_err(|_| {
        actix_web::error::ErrorBadRequest("Invalid notification ID format")
    })?;
    
    let status = notification_service.get_notification_status(notification_id).await
        .map_err(|e| {
            match e {
...
async fn create_subscription(
    ...
    let result = notification_service.create_subscription(request.into_inner()).await
        .map_err(|e| {
