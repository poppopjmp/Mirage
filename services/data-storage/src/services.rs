use crate::models::{DataEntity, Relationship, StoreDataRequest, StoreRelationshipRequest, QueryParams};
use crate::repositories::{DbPool, DataRepository};
use chrono::Utc;
...
impl StorageService {
    pub fn new(db_pool: DbPool, mongo_db: Database, es_client: Elasticsearch, es_index_prefix: String) -> Self {
        Self {
            repo: Arc::new(DataRepository::new(db_pool, mongo_db, es_client, es_index_prefix.clone())),
            es_index_prefix,
        }
    }
    ...
    pub async fn store_data(&self, req: StoreDataRequest) -> Result<Uuid> {
        ...
        if req.value.is_empty() {
            return Err(Error::Validation("Entity value cannot be empty".to_string()));
        }
    ...
    pub async fn get_data(&self, id: &Uuid) -> Result<DataEntity> {
        let entity = self.repo.get_entity(id).await?
    ...
    pub async fn update_data(&self, id: &Uuid, data: serde_json::Value) -> Result<()> {
        // Get existing entity
        let mut entity = self.repo.get_entity(id).await?
            .ok_or_else(|| Error::NotFound(format!("Entity with ID {} not found", id)))?;
    ...
    pub async fn create_relationship(&self, req: StoreRelationshipRequest) -> Result<Uuid> {
        ...
        if req.relationship_type.is_empty() {
            return Err(Error::Validation("Relationship type cannot be empty".to_string()));
        }
    ...
    pub async fn get_relationships_for_entity(&self, entity_id: &Uuid) -> Result<Vec<Relationship>> {
