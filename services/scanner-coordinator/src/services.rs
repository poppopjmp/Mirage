...
use crate::models::{
    ...
    CreateScanRequest, UpdateScanRequest, ScanResponse, ScanDetailResponse, ScanTargetResponse,
    ScanModuleResponse, CreateTargetRequest, ModuleRequest
};
use crate::repositories::{ScanRepository, ScanTargetRepository, ScanModuleRepository};
...
impl ScannerService {
    ...
    /// Create a new scan
    pub async fn create_scan(&self, request: CreateScanRequest, user_id: Option<Uuid>) -> Result<ScanResponse> {
        // Validate request
        ...
        for target in &targets {
            self.target_repo.create_target(target).await
                .map_err(|e| Error::from(e))?;
        }
        
        // Store modules in database
        for module in &modules {
            self.module_repo.create_module(module).await
                .map_err(|e| Error::from(e))?;
        }
        
        // If no schedule is provided, queue scan for immediate execution
        if request.schedule.is_none() {
            self.scheduler.enqueue_scan(scan_id, priority).await
                .map_err(|e| Error::from(e))?;
        }
    ...
    pub async fn get_scan(&self, scan_id: Uuid) -> Result<ScanDetailResponse> {
        // Get scan
        let scan = self.scan_repo.get_scan_by_id(scan_id).await
            .map_err(|e| Error::from(e))?
        ...
        // Get modules
        let modules = self.module_repo.get_modules_for_scan(scan_id).await
            .map_err(|e| Error::from(e))?;
        ...
        let target_responses: Vec<ScanTargetResponse> = targets.into_iter()
        ...
        let module_responses: Vec<ScanModuleResponse> = modules.into_iter()
    ...
    pub async fn list_scans(
        ...
        // Query database
        let (scans, total) = self.scan_repo.list_scans(
            status.as_ref(),
            created_by.as_ref(),
            tag.as_deref(),
        ...
        let responses = scans.into_iter().map(|s| {
    ...
    pub async fn update_scan(&self, scan_id: Uuid, request: UpdateScanRequest) -> Result<ScanResponse> {
        // Get current scan
        let mut scan = self.scan_repo.get_scan_by_id(scan_id).await
            .map_err(|e| Error::from(e))?
            .ok_or_else(|| Error::NotFound(format!("Scan with ID {} not found", scan_id)))?;
    ...
    pub async fn start_scan(&self, scan_id: Uuid) -> Result<ScanResponse> {
        ...
        self.scheduler.enqueue_scan(scan_id, scan.priority).await
            .map_err(|e| Error::from(e))?;
        ...
        self.get_scan(scan_id).await.map(|detail| ScanResponse {
            ...
            completed_targets: detail.targets.iter().filter(|t| t.status == ScanTargetStatus::Completed).count() as i32,
    ...
    async fn create_scan_targets(&self, scan_id: Uuid, requests: &[CreateTargetRequest]) -> Result<Vec<ScanTarget>> {
    ...
    async fn create_scan_modules(&self, scan_id: Uuid, requests: &[ModuleRequest]) -> Result<Vec<ScanModule>> {
        let mut modules = Vec::with_capacity(requests.len());
        
        for (index, request) in requests.iter().enumerate() {
            // Fetch module info to validate it exists
            let module_info = self.integration.get_module_info(&request.module_id).await
                .map_err(|e| Error::from(e))?;
