...
use crate::models::{
    CreateScanRequest, UpdateScanRequest, ScanStatus, ScanQueryParams,
    AddTargetRequest, AddModuleRequest
};
...
async fn create_scan(
    ...
    let result = service.create_scan(request.into_inner(), user_id).await
        .map_err(|e| {
...
async fn get_scan(
    ...
    let scan_id = Uuid::parse_str(&id).map_err(|_| {
        actix_web::error::ErrorBadRequest("Invalid scan ID format")
    })?;
    
    let scan = service.get_scan(scan_id).await
        .map_err(|e| {
            match e {
...
async fn list_scans(
    ...
    let (scans, total) = service.list_scans(
        status,
        created_by,
        tag,
        created_after,
...
async fn update_scan(
    ...
    let scan = service.update_scan(scan_id, request.into_inner()).await
        .map_err(|e| {
            match e {
                CommonError::NotFound(_) => actix_web::error::ErrorNotFound(e),
                CommonError::Validation(_) => actix_web::error::ErrorBadRequest(e),
...
async fn start_scan(
    ...
    let scan = service.start_scan(scan_id).await
        .map_err(|e| {
            match e {
                CommonError::NotFound(_) => actix_web::error::ErrorNotFound(e),
                CommonError::Validation(_) => actix_web::error::ErrorBadRequest(e),
...
async fn cancel_scan(
    ...
    let scan = service.cancel_scan(scan_id).await
        .map_err(|e| {
            match e {
                CommonError::NotFound(_) => actix_web::error::ErrorNotFound(e),
                CommonError::Validation(_) => actix_web::error::ErrorBadRequest(e),
...
async fn add_targets(
    ...
    Err(actix_web::error::ErrorNotImplemented("Adding targets to an existing scan is not yet implemented"))
}
...
async fn add_modules(
    ...
    // For now, return a not implemented error
    Err(actix_web::error::ErrorNotImplemented("Adding modules to an existing scan is not yet implemented"))
}
