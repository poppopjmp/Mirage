...
use crate::models::{Scan, ScanStatus, ScanTarget, ScanTargetStatus, ScanModule, ScanModuleStatus};
...
impl SchedulerService {
    ...
    pub async fn enqueue_scan(&self, scan_id: Uuid, priority: i32) -> ScannerResult<()> {
        ...
        conn.zadd::<_, _, _, i64>(SCAN_QUEUE_KEY, scan_id.to_string(), priority).await?;
    ...
    pub async fn start_scan(&self, scan_id: Uuid) -> ScannerResult<()> {
        ...
        let scan = match self.scan_repo.get_scan_by_id(scan_id).await? {
            Some(scan) => scan,
            None => return Err(ScannerError::NotFound(format!("Scan {} not found", scan_id))),
        };
    ...
    async fn enqueue_scan_targets(&self, scan_id: Uuid, targets: &[ScanTarget]) -> ScannerResult<()> {
        let mut conn = self.redis_client.get_async_connection().await?;
        let queue_key = format!("{}{}", TARGET_QUEUE_PREFIX, scan_id);
        
        // Add target IDs to scan's target queue
        for target in targets {
            conn.rpush::<_, _, i64>(&queue_key, target.id.to_string()).await?;
        }
    ...
    pub async fn get_next_target(&self, scan_id: Uuid) -> ScannerResult<Option<ScanTarget>> {
        ...
        if let Some(target_id_str) = target_id {
            ...
            let target_id = match Uuid::parse_str(&target_id_str) {
                Ok(id) => id,
                Err(_) => return Err(ScannerError::Internal(format!(
    ...
    pub async fn process_target(&self, target: &ScanTarget, module_id: Uuid) -> ScannerResult<()> {
        ...
        let task_id = self.integration_service.create_collection_task(
            target,
            module_id,
            None, // parameters
            None, // priority
    ...
    /// Complete a scan
    pub async fn complete_scan(&self, scan_id: Uuid, success: bool, error_message: Option<String>) -> ScannerResult<()> {
    ...
    pub async fn check_scan_completion(&self, scan_id: Uuid) -> ScannerResult<bool> {
        ...
        let completed = targets.iter().filter(|t| 
            t.status == ScanTargetStatus::Completed || 
            t.status == ScanTargetStatus::Failed ||
            t.status == ScanTargetStatus::Skipped
        ).count();
        ...
        if total > 0 {
            ...
            self.scan_repo.update_scan_status(
                scan_id,
                if all_processed { ScanStatus::Completed } else { ScanStatus::Running },
                None,
                if all_processed { Some(Utc::now()) } else { None },
...
async fn process_scan_targets(scheduler: &SchedulerService, scan_id: Uuid) -> ScannerResult<()> {
    ...
    let modules = scheduler.scan_repo.get_scan_by_id(scan_id).await?
        .map(|s| {
    ...
    if modules.is_empty() {
        return Err(ScannerError::Validation("No modules configured for scan".into()));
    }
    ...
    loop {
        ...
        for module_id in &modules {
            if let Err(e) = scheduler.process_target(&target, *module_id).await {
                tracing::error!("Failed to process target {} with module {}: {}", target.id, module_id, e);
                success = false;
            }
        }
    }
