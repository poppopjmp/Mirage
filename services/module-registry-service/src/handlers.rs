...
use mirage_common::{Error as CommonError, models::Module};
...
async fn list_modules(
    ...
    let modules = module_service.list_modules(query.limit.unwrap_or(100), query.offset.unwrap_or(0)).await
        .map_err(|e| {
...
async fn get_module(
    ...
    let id = Uuid::parse_str(&id).map_err(|_| {
        actix_web::error::ErrorBadRequest("Invalid module ID")
    })?;
    
    let module = module_service.get_module(&id).await
        .map_err(|e| {
            match e {
...
async fn register_module(
    ...
    let module = module_service.register_module(data.into_inner()).await
        .map_err(|e| {
...
async fn update_module(
    ...
    let module = module_service.update_module(&id, data.into_inner()).await
        .map_err(|e| {
            match e {
                CommonError::NotFound(_) => actix_web::error::ErrorNotFound(e),
                CommonError::Validation(_) => actix_web::error::ErrorBadRequest(e),
...
async fn delete_module(
    ...
    module_service.delete_module(&id).await
        .map_err(|e| {
            match e {
                CommonError::NotFound(_) => actix_web::error::ErrorNotFound(e),
                _ => {
