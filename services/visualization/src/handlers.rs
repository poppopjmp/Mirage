...
async fn get_visualization(
    ...
    let visualization_id = Uuid::parse_str(&id).map_err(|_| {
        actix_web::error::ErrorBadRequest("Invalid visualization ID format")
    })?;
    
    let result = viz_service.get_visualization(&visualization_id).await
        .map_err(|e| {
            match e {
                CommonError::NotFound(_) => actix_web::error::ErrorNotFound(e),
                _ => {
...
async fn render_visualization(
    ...
    let width = query.width.unwrap_or(config.visualization.default_graph_width);
    let height = query.height.unwrap_or(config.visualization.default_graph_height);
    
    let result = viz_service.render_visualization(&visualization_id, &format, width, height).await
        .map_err(|e| {
            match e {
                CommonError::NotFound(_) => actix_web::error::ErrorNotFound(e),
                CommonError::Validation(_) => actix_web::error::ErrorBadRequest(e),
...
async fn list_visualizations(
    ...
    let result = viz_service.list_visualizations().await
        .map_err(|e| {
...
async fn create_report(
    ...
    let result = viz_service.generate_report(request.into_inner()).await
        .map_err(|e| {
