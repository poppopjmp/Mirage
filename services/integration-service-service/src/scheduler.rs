use crate::models::{
    Integration, ExecutionRecord, ExecutionStatus, IntegrationStatus, ScheduleType, Credential
};
use crate::repositories::{IntegrationRepository, ExecutionRepository};
...
impl SchedulerService {
    ...
    pub async fn execute_scheduled_integration(&self, integration: &Integration) -> IntegrationResult<()> {
        info!(
        ...
        self.execution_repo.create_execution_record(&execution).await?;
        
        // Acquire execution lock
        let lock_key = format!("{}:integration:lock:{}", self.config.redis.prefix, integration.id);
        let mut redis_conn = self.redis_client.get_async_connection().await?;
        
        // Try to acquire lock with 10-second expiry (to prevent abandoned locks)
        let lock_result: bool = redis_conn.set_nx(&lock_key, execution_id.to_string()).await?;
        if !lock_result {
            warn!("Integration {} is already being executed by another process", integration.id);
            
            // Update execution record
            execution.status = ExecutionStatus::Failed;
            execution.completed_at = Some(Utc::now());
            execution.error_message = Some("Integration is already being executed by another process".to_string());
            self.execution_repo.update_execution_record(&execution).await?;
            
            return Err(IntegrationError::Scheduling("Integration is already being executed".into()));
        }
        ...
        match result {
            Ok((result_count, _)) => {
                ...
                updated_integration.next_execution = self.calculate_next_execution(&updated_integration);
                ...
                // Update integration
                self.integration_repo.update_integration(&updated_integration).await?;
            },
            Err(e) => {
                error!(
    ...
    pub async fn execute_integration_manually(
        ...
        // Get integration
        let integration = match self.integration_repo.get_integration_by_id(integration_id).await? {
            Some(integration) => integration,
            None => return Err(IntegrationError::NotFound(format!(
                "Integration {} not found", integration_id
            ))),
        };
        ...
        // Execute integration
        let start_time = Instant::now();
        let result = self.execute_integration_with_params(&integration, execution_id, parameters.as_ref(), target.as_deref()).await;
        let elapsed = start_time.elapsed();
    ...
    async fn execute_integration(&self, integration: &Integration, execution_id: Uuid) -> IntegrationResult<(Option<i32>, Option<String>)> {
        // Just delegate to the parameterized version with no parameters or target
        self.execute_integration_with_params(integration, execution_id, None, None).await
    }
    ...
    async fn execute_integration_with_params(
        ...
        let provider = self.provider_registry.get_provider(&integration.provider_id)
            .ok_or_else(|| IntegrationError::Provider(format!(
                "Provider {} not found", integration.provider_id
            )))?;
        
        // Get credentials if any
        let credentials = self.get_credentials_for_integration(&integration.id).await?;
        ...
        let result = provider.execute(
            ...
            &|data| {
                if let Some(cred) = credential {
                    ...
                    Err(IntegrationError::Authentication("No credentials available".into()))
                }
            },
        ).await?;
    ...
    async fn get_credentials_for_integration(&self, integration_id: &Uuid) -> IntegrationResult<Vec<Credential>> {
    ...
    fn decrypt_credential_data(&self, credential: &Credential, data: &str) -> IntegrationResult<String> {
...
pub async fn run_scheduler(scheduler: SchedulerService) {
    ...
    loop {
        ...
        match scheduler.integration_repo.get_scheduled_integrations().await {
            Ok(integrations) => {
                info!("Found {} scheduled integrations to execute", integrations.len());
