use crate::models::{
    ProviderInfo, AuthType, Credential, Integration, ExecutionRecord, ExecutionStatus
};
...
impl Provider for HttpApiProvider {
    ...
    fn validate_config(&self, config: &Value) -> IntegrationResult<()> {
        ...
        if let Some(obj) = config.as_object() {
            if !obj.contains_key("base_url") {
                return Err(IntegrationError::Validation("Missing required field: base_url".into()));
            }
            
            if !obj.contains_key("method") {
                return Err(IntegrationError::Validation("Missing required field: method".into()));
            }
        } else {
            return Err(IntegrationError::Validation("Configuration must be an object".into()));
        }
    ...
    async fn execute(
        ...
        // Extract config
        let config = integration.config.as_object().ok_or_else(|| 
            IntegrationError::Validation("Invalid configuration format".into())
        )?;
        ...
        let mut req_builder = match method {
            ...
            _ => return Err(IntegrationError::Validation(format!("Unsupported method: {}", method))),
        };
        ...
        if let Some(cred) = credential {
            ...
            match cred.auth_type {
                AuthType::ApiKey => {
                    ...
                    let api_key_auth: Result<crate::models::ApiKeyAuth, _> = serde_json::from_str(&auth_data);
                    if let Ok(api_key) = api_key_auth {
                        ...
                        return Err(IntegrationError::Authentication("Invalid API key format".into()));
                    }
                },
                AuthType::Basic => {
                    // Parse Basic auth data
                    let basic_auth: Result<crate::models::BasicAuth, _> = serde_json::from_str(&auth_data);
                    if let Ok(basic) = basic_auth {
                        req_builder = req_builder.basic_auth(basic.username, Some(basic.password));
                    } else {
                        return Err(IntegrationError::Authentication("Invalid Basic auth format".into()));
                    }
                },
                AuthType::Bearer => {
                    // Parse Bearer auth data
                    let bearer_auth: Result<crate::models::BearerAuth, _> = serde_json::from_str(&auth_data);
                    if let Ok(bearer) = bearer_auth {
                        req_builder = req_builder.header("Authorization", format!("Bearer {}", bearer.token));
                    } else {
                        return Err(IntegrationError::Authentication("Invalid Bearer auth format".into()));
                    }
                },
                AuthType::OAuth2 => {
                    // Parse OAuth2 auth data
                    let oauth2_auth: Result<crate::models::OAuth2Auth, _> = serde_json::from_str(&auth_data);
                    if let Ok(oauth2) = oauth2_auth {
                        let token_type = oauth2.token_type.unwrap_or_else(|| "Bearer".to_string());
                        req_builder = req_builder.header("Authorization", format!("{} {}", token_type, oauth2.access_token));
                    } else {
                        return Err(IntegrationError::Authentication("Invalid OAuth2 auth format".into()));
                    }
                },
                _ => return Err(IntegrationError::Authentication(format!("Unsupported auth type: {:?}", cred.auth_type))),
            }
        }
        ...
        let response_data: Value = serde_json::from_str(&response_body)
            .map_err(|e| IntegrationError::ExternalApi(format!("Failed to parse response as JSON: {}", e)))?;
...
impl Provider for TwitterProvider {
        ...
        if let Some(obj) = config.as_object() {
            if let Some(api_version) = obj.get("api_version").and_then(|v| v.as_str()) {
                if api_version != "v1" && api_version != "v2" {
                    return Err(IntegrationError::Validation("api_version must be either 'v1' or 'v2'".into()));
                }
            }
            
            if let Some(max_results) = obj.get("max_results").and_then(|v| v.as_i64()) {
                if max_results < 5 || max_results > 100 {
                    return Err(IntegrationError::Validation("max_results must be between 5 and 100".into()));
                }
            }
        }
    ...
    async fn execute(
        ...
        let endpoint = match target {
            ...
            _ => return Err(IntegrationError::Validation("Invalid or missing target type".into())),
        };
        ...
        let auth_header = match cred.auth_type {
            AuthType::Bearer => {
                ...
                match bearer_auth {
                    ...
                    Err(_) => return Err(IntegrationError::Authentication("Invalid Bearer auth format".into())),
                }
            },
            AuthType::OAuth2 => {
                let oauth2_auth: Result<crate::models::OAuth2Auth, _> = serde_json::from_str(&auth_data);
                match oauth2_auth {
                    ...
                    Err(_) => return Err(IntegrationError::Authentication("Invalid OAuth2 auth format".into())),
                }
            },
            _ => return Err(IntegrationError::Authentication("Unsupported auth type for Twitter provider".into())),
        };
        ...
        if let Some(tweet_fields) = config.get("tweet_fields").and_then(|v| v.as_array()) {
            let fields: Vec<String> = tweet_fields.iter()
        ...
        match target {
            ...
            Some("hashtag") => {
                ...
                if let Some(hashtag) = parameters.and_then(|p| p.get("hashtag").and_then(|v| v.as_str())) {
            ...
            Some("tweet") => {
                // Tweet ID parameter
                if let Some(tweet_id) = parameters.and_then(|p| p.get("id").and_then(|v| v.as_str())) {
            ...
            Some(target_type) => {
                ...
                if target_type == "followers" || target_type == "following" {
                    if let Some(user_id) = parameters.and_then(|p| p.get("user_id").and_then(|v| v.as_str())) {
                        let url = url.replace("{id}", user_id);
                        req_builder = client.get(&url);
                    } else {
                        return Err(IntegrationError::Validation("user_id parameter required".into()));
                    }
                }
            },
            _ => {}
        ...
        let result_count = if let Some(data) = response_data.get("data") {
            ...
            meta.get("result_count").and_then(|v| v.as_i64()).map(|v| v as i32)
        } else {
            None
        };
