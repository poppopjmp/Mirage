...
use crate::models::{
    CreateScanModuleRequest, CreateScanRequest, CreateScanTargetRequest, Scan, ScanModule,
    ScanModuleResult, ScanModuleResultsSummary, ScanModuleStatus, ScanStatus, ScanTarget,
    ScanTargetStatus, UpdateScanRequest,
};
use crate::repositories::{ModuleRepository, ResultRepository, ScanRepository};
use chrono::Utc;
...
impl ScanService {
    ...
    pub async fn create_scan(&self, request: CreateScanRequest, user_id: Uuid) -> Result<Scan> {
        ...
        if request.scheduled_at.is_none() {
            self.scan_tx
                .send(ScanWorkerMessage::StartScan(scan_id))
                .await
    ...
    pub async fn cancel_scan(&self, scan_id: Uuid) -> Result<Scan> {
        ...
        self.scan_tx
            .send(ScanWorkerMessage::CancelScan(scan_id))
            .await
    ...
    async fn create_scan_targets(
        &self,
        scan_id: Uuid,
        target_requests: &[CreateScanTargetRequest],
    ...
    async fn create_scan_modules(
        &self,
        scan_id: Uuid,
        module_requests: &[CreateScanModuleRequest],
        ...
        for (idx, request) in ordered_requests.iter().enumerate() {
            ...
            let module_info = self
                ...
                .await?
                .ok_or_else(|| {
                    Error::NotFound(format!("Module with ID {} not found", request.module_id))
                })?;
    ...
    pub(crate) async fn complete_scan_execution(
        &self,
        scan_id: Uuid,
        success: bool,
        ...
        self.scan_repo
            .update_scan_status(scan_id, status, None, None, Some(Utc::now()), error_message)
            .await?;

        Ok(())
    }

    pub(crate) async fn register_active_scan(
        &self,
        scan_id: Uuid,
