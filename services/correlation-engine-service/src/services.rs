...
use crate::models::{
    EntityNode, Relationship, CorrelationRequest, CorrelationResult, PatternMatchRequest,
    PatternMatchResult, PatternMatch, AnalysisJob, AnalysisJobType, JobStatus, CorrelationParameters,
    BatchCorrelationRequest, GraphNode, GraphRelationship, PathFindingRequest, PathFindingResult,
    EntityPath, CorrelationInsight, EntityImportance
};
use crate::repositories::{GraphRepository, DataStorageRepository, GraphDatabase};
...
impl CorrelationService {
    ...
    pub async fn import_entity(&self, entity_id: &Uuid) -> Result<EntityNode> {
        ...
        let entity_type = entity_data["entity_type"].as_str()
            .ok_or_else(|| Error::ExternalApi("Missing entity_type in data".to_string()))?
            .to_string();
            
        let value = entity_data["value"].as_str()
        ...
        for rel_data in relationships {
            ...
            let rel_type = rel_data["relationship_type"].as_str()
    ...
    pub async fn correlate(&self, req: CorrelationRequest) -> Result<CorrelationResult> {
        ...
        let max_depth = req.max_depth.unwrap_or(self.config.analysis.max_correlation_depth);
        let min_confidence = req.min_confidence.unwrap_or(self.config.analysis.min_confidence_score);
        
        // Get correlated entities and relationships
        let (nodes, relationships) = self.graph_repo.get_neighbors(
            &req.entity_id, 
            max_depth,
            min_confidence
        ).await?;
        ...
        let pattern_matches = match req.pattern_type.as_str() {
            "ip_domain_relationship" => {
                analysis::patterns::find_ip_domain_relationships(
                    ...
                    req.min_confidence.unwrap_or(self.config.analysis.min_confidence_score),
            ...
            _ => return Err(Error::Validation(format!("Unknown pattern type: {}", req.pattern_type))),
        };
    ...
    pub async fn enrich_entity(&self, req: &EntityEnrichmentRequest) -> Result<EntityNode> {
        // Get the entity node
        let entity = self.graph_repo.get_entity_by_id(&req.entity_id).await?
            .ok_or_else(|| Error::NotFound(format!("Entity with ID {} not found", req.entity_id)))?;
            ...
            match enrichment_type.as_str() {
                "domain_whois" => {
                    if entity.entity_type == "domain" {
                        updated = analysis::enrichment::enrich_domain_whois(
                            &mut enriched_entity
                        ).await?;
                    }
                },
                "ip_geolocation" => {
                    if entity.entity_type == "ip" {
                        updated = analysis::enrichment::enrich_ip_geolocation(
                            &mut enriched_entity
                        ).await?;
                    }
                },
                "email_breach_check" => {
                    if entity.entity_type == "email" {
                        updated = analysis::enrichment::enrich_email_breach_check(
                            &mut enriched_entity
                        ).await?;
                    }
                },
                _ => return Err(Error::Validation(format!("Unknown enrichment type: {}", enrichment_type))),
            }
        }
    ...
    pub async fn generate_correlation(&self, request: CorrelationRequest) -> Result<CorrelationResult> {
        // Validate that entity exists
        let entity = self.graph_db.get_node(&request.entity_id).await?
            .ok_or_else(|| Error::NotFound(format!("Entity with ID {} not found", request.entity_id)))?;
            
        // Set up defaults
        let max_depth = request.max_depth.unwrap_or_else(|| self.config.engine.max_correlation_depth);
        let min_confidence = request.min_confidence.unwrap_or_else(|| self.config.engine.min_correlation_confidence);
        let max_entities = request.max_entities.unwrap_or_else(|| self.config.engine.max_entities_per_correlation);
            
        // Fetch connected nodes up to depth
        let nodes = self.graph_db.get_connected_nodes(&request.entity_id, max_depth, max_entities).await?;
        ...
        for node in &all_nodes {
            let node_relationships = self.graph_db.get_relationships(&node.id, max_depth, max_entities).await?;
            ...
            for rel in node_relationships {
                   ...
                   node_ids.contains(&rel.target_id)
                {
                    ...
                    if include && !exclude && !relationships.iter().any(|r: &GraphRelationship| r.id == rel.id) {
        ...
        // Generate insights
        let insights = self.analyzer.analyze_correlation(&all_nodes, &relationships);
    ...
    pub async fn generate_batch_correlation(&self, request: BatchCorrelationRequest) -> Result<AnalysisJob> {
        // Create job
        ...
        let job = AnalysisJob {
            ...
            parameters: HashMap::from_iter([
                ("max_depth".to_string(), serde_json::to_value(request.max_depth)?),
                ("min_confidence".to_string(), serde_json::to_value(request.min_confidence)?),
                ("include_entities".to_string(), serde_json::to_value(request.include_entities)?),
                ("exclude_entities".to_string(), serde_json::to_value(request.exclude_entities)?),
                ("include_relationships".to_string(), serde_json::to_value(request.include_relationships)?),
                ("exclude_relationships".to_string(), serde_json::to_value(request.exclude_relationships)?),
                ("max_entities".to_string(), serde_json::to_value(request.max_entities)?),
            ]),
        ...
        tokio::spawn(async move {
            ...
            for entity_id in request_clone.entity_ids {
                ...
                match service_clone.generate_correlation(single_request).await {
                    ...
                    Err(e) => {
                        tracing::error!("Failed to generate correlation for entity {}: {}", entity_id, e);
                        failed = true;
                        error_message = Some(format!("Error processing entity {}: {}", entity_id, e));
                    }
                }
            }
    ...
    pub async fn find_path(&self, request: PathFindingRequest) -> Result<PathFindingResult> {
        ...
        let source_entity = self.graph_db.get_node(&request.source_entity_id).await?
            .ok_or_else(|| Error::NotFound(format!("Source entity with ID {} not found", request.source_entity_id)))?;
            
        let target_entity = self.graph_db.get_node(&request.target_entity_id).await?
            .ok_or_else(|| Error::NotFound(format!("Target entity with ID {} not found", request.target_entity_id)))?;
        ...
        let raw_paths = self.graph_db.find_paths(
    ...
    fn filter_nodes_by_type(
        ...
        let filtered = nodes.into_iter().filter(|node| {
    ...
    fn filter_paths(
        ...
        if include_entity_types.is_none() && exclude_entity_types.is.none() && 
           include_relationship_types.is.none() && exclude_relationship_types.is.none() {
            return Ok(paths);
        }
        
        let filtered = paths.into_iter().filter(|path| {
            // Check entities
