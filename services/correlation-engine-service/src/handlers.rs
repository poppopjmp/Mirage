...
use crate::models::{BatchCorrelationRequest, CorrelationRequest, PathFindingRequest};
...
async fn correlate_entity(
    ...
    let result = correlation_service
        .generate_correlation(request.into_inner())
        .await
        .map_err(|e| {
...
async fn batch_correlate(
    ...
    let job = correlation_service
        .generate_batch_correlation(request.into_inner())
        .await
        .map_err(|e| {
...
async fn get_job(
    ...
    let job_id = Uuid::parse_str(&id)
        .map_err(|_| actix_web::error::ErrorBadRequest("Invalid job ID format"))?;

    let status = correlation_service
    ...
    match status {
        ...
        None => Err(actix_web::error::ErrorNotFound(CommonError::NotFound(
            format!("Job with ID {} not found", job_id),
        ))),
    }
}
...
async fn get_correlation_result(
    ...
    let result_id = Uuid::parse_str(&id)
        .map_err(|_| actix_web::error::ErrorBadRequest("Invalid result ID format"))?;
