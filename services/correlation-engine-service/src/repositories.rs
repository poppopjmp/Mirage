use crate::config::{Neo4jConfig, DataStorageConfig, GraphDatabaseConfig};
use crate::models::{EntityNode, Relationship, AnalysisJob, GraphNode, GraphRelationship, EntityImportance, CorrelationResult, PathFindingResult};
...
use chrono::Utc;
use gremlin_client::{GremlinClient, process::traversal::{GraphTraversalSource, __}};
...
impl GraphRepository {
    ...
    pub async fn get_entity_by_id(&self, id: &Uuid) -> Result<Option<EntityNode>> {
        ...
        if let Some(row) = result.next().await
            ...
            let properties_json: String = node.get("properties")
            ...
            let properties: HashMap<String, serde_json::Value> = serde_json::from_str(&properties_json)
                .map_err(|e| Error::Internal(format!("Failed to deserialize properties: {}", e)))?;
            ...
            let entity = EntityNode {
                ...
                entity_type: node.get("entity_type")
                    .map_err(|_| Error::Database("Failed to get entity_type from node".to_string()))?,
                value: node.get("value")
                    .map_err(|_| Error::Database("Failed to get value from node".to_string()))?,
                properties,
                confidence: node.get::<i64>("confidence")
                    .map_err(|_| Error::Database("Failed to get confidence from node".to_string()))? as u8,
                created_at: chrono::DateTime::parse_from_rfc3339(&node.get::<String>("created_at")
                    .map_err(|_| Error::Database("Failed to get created_at from node".to_string()))?)
                    .map_err(|_| Error::Database("Invalid DateTime format".to_string()))?.with_timezone(&Utc),
            };
    ...
    pub async fn get_neighbors(&self, entity_id: &Uuid, depth: u8, min_confidence: u8) -> Result<(Vec<EntityNode>, Vec<Relationship>)> {
        let query = Query::new(
        ...
        while let Some(row) = result.next().await
            ...
            let node_id = node.get::<String>("id")
                .map_err(|_| Error::Database("Failed to get id from node".to_string()))?;
            ...
            let rels: Vec<Relation> = row.get("rels")
                .map_err(|e| Error::Database(format!("Failed to get relationships from row: {}", e)))?;
                
            for rel in rels {
                let rel_id = rel.get::<String>("id")
                    .map_err(|_| Error::Database("Failed to get id from relationship".to_string()))?;
                    
                if !seen_rels.contains_key(&rel_id) {
                    seen_rels.insert(rel_id, true);
                    
                    let properties_json: String = rel.get("properties")
                        .map_err(|_| Error::Database("Failed to get properties from relationship".to_string()))?;
                        
                    let properties: HashMap<String, serde_json::Value> = serde_json::from_str(&properties_json)
                        .map_err(|e| Error::Internal(format!("Failed to deserialize properties: {}", e)))?;
                        
                    // Get start and end node IDs for this relationship
                    let start_node_id = rel.start_node_id()
                        .map_err(|e| Error::Database(format!("Failed to get start node ID: {}", e)))?;
                        
                    let end_node_id = rel.end_node_id()
                        .map_err(|e| Error::Database(format!("Failed to get end node ID: {}", e)))?;
                    ...
                    let relationship = Relationship {
                        id: Uuid::parse_str(&rel_id).map_err(|_| Error::Database("Invalid UUID format".to_string()))?,
                        source_id,
                        target_id,
                        relationship_type: rel.get("relationship_type")
                            .map_err(|_| Error::Database("Failed to get relationship_type".to_string()))?,
                        properties,
                        confidence: rel.get::<i64>("confidence")
                            .map_err(|_| Error::Database("Failed to get confidence from relationship".to_string()))? as u8,
                        created_at: chrono::DateTime::parse_from_rfc3339(&rel.get::<String>("created_at")
                            .map_err(|_| Error::Database("Failed to get created_at from relationship".to_string()))?)
                            .map_err(|_| Error::Database("Invalid DateTime format".to_string()))?.with_timezone(&Utc),
                    };
    ...
    async fn get_entity_id_by_internal_id(&self, internal_id: &i64) -> Result<Uuid> {
        let query = Query::new(
            "MATCH (n) WHERE id(n) = $internal_id RETURN n.id as entity_id"
        )
        ...
        if let Some(row) = result.next().await
            ...
            Ok(Uuid::parse_str(&entity_id).map_err(|_| Error::Database("Invalid UUID format".to_string()))?)
        } else {
            Err(Error::NotFound(format!("Entity with internal ID {} not found", internal_id)))
        }
    }
}
...
impl AnalysisJobRepository {
    ...
    pub async fn update_job_status(&self, job_id: &Uuid, status: crate::models::JobStatus, result_id: Option<Uuid>, error: Option<String>) -> Result<()> {
        let collection = self.db.collection::<mongodb::bson::Document>("analysis_jobs");
        ...
        if status == crate::models::JobStatus::Completed || status == crate::models::JobStatus::Failed {
...
impl DataStorageRepository {
    ...
    pub async fn get_entity(&self, id: &Uuid) -> Result<serde_json::Value> {
        ...
        let response = self.client.get(&url)
            .send()
            .await
            .map_err(|e| Error::ExternalApi(format!("Failed to fetch entity from data storage: {}", e)))?;
    ...
    pub async fn get_relationships(&self, entity_id: &Uuid) -> Result<Vec<serde_json::Value>> {
        ...
        let response = self.client.get(&url)
            .send()
            .await
            .map_err(|e| Error::ExternalApi(format!("Failed to fetch relationships from data storage: {}", e)))?;
        ...
        let data = response.json::<Vec<serde_json::Value>>().await
            .map_err(|e| Error::ExternalApi(format!("Failed to parse relationship data: {}", e)))?;
            
        Ok(data)
    }
    
    pub async fn query_entities(&self, query_params: &serde_json::Value) -> Result<Vec<serde_json::Value>> {
        ...
        let response = self.client.get(&url)
            ...
            .send()
            .await
            .map_err(|e| Error::ExternalApi(format!("Failed to query entities from data storage: {}", e)))?;
...
impl GraphDatabase {
    ...
    pub async fn get_node(&self, entity_id: &Uuid) -> Result<Option<GraphNode>> {
        ...
        match result {
            Some(value_map) => {
                ...
                let entity_type = self.extract_string_property(&value_map, "entity_type")?
                    .unwrap_or_default();
                    
                let value = self.extract_string_property(&value_map, "value")?
                    .unwrap_or_default();
                    
                let source = self.extract_string_property(&value_map, "source")?
                    .unwrap_or_default();
                
                let confidence = self.extract_int_property(&value_map, "confidence")?
                    .unwrap_or(0) as u8;
                    
                let first_seen = self.extract_date_property(&value_map, "first_seen")?
                    .unwrap_or_else(Utc::now);
                    
                let last_seen = self.extract_date_property(&value_map, "last_seen")?
                    .unwrap_or_else(Utc::now);
                    
                let data = self.extract_data_properties(&value_map)?;
                
                let importance = self.extract_float_property(&value_map, "importance")?
                    .unwrap_or(1.0);
    ...
    pub async fn get_relationships(&self, entity_id: &Uuid, depth: i32, max_entities: i32) -> Result<Vec<GraphRelationship>> {
        let id_str = entity_id.to_string();
        ...
        for rel_map in results {
            let relationship_type = self.extract_string_property(&rel_map, "relationship_type")?
                .unwrap_or_default();
                
            let source_id_str = self.extract_string_property(&rel_map, "source_id")?
                .unwrap_or_default();
                
            let target_id_str = self.extract_string_property(&rel_map, "target_id")?
                .unwrap_or_default();
                
            let rel_id_str = self.extract_string_property(&rel_map, "id")?
            ...
            let source = self.extract_string_property(&rel_map, "source")?
                .unwrap_or_default();
                
            let confidence = self.extract_int_property(&rel_map, "confidence")?
                .unwrap_or(0) as u8;
                
            let first_seen = self.extract_date_property(&rel_map, "first_seen")?
                .unwrap_or_else(Utc::now);
                
            let last_seen = self.extract_date_property(&rel_map, "last_seen")?
                .unwrap_or_else(Utc::now);
                
            let data = self.extract_data_properties(&rel_map)?;
                
            let strength = self.extract_float_property(&rel_map, "strength")?
                .unwrap_or(1.0);
    ...
    pub async fn get_connected_nodes(&self, entity_id: &Uuid, depth: i32, max_entities: i32) -> Result<Vec<GraphNode>> {
        let id_str = entity_id.to_string();
        ...
        for node_map in results {
            let id_str = self.extract_string_property(&node_map, "id")?
                .unwrap_or_else(|| Uuid::new_v4().to_string());
    ...
    fn extract_string_property(&self, value_map: &gremlin_client::GValue, key: &str) -> Result<Option<String>> {
    ...
    fn extract_int_property(&self, value_map: &gremlin_client::GValue, key: &str) -> Result<Option<i32>> {
        if let Some(values) = value_map.get(key) {
            if let Some(value) = values.get(0) {
    ...
    fn extract_float_property(&self, value_map: &gremlin_client::GValue, key: &str) -> Result<Option<f32>> {
        if let Some(values) = value_map.get(key) {
            if let Some(value) = values.get(0) {
    ...
    fn extract_date_property(&self, value_map: &gremlin_client::GValue, key: &str) -> Result<Option<DateTime<Utc>>> {
        if let Some(values) = value_map.get(key) {
            if let Some(value) = values.get(0) {
    ...
    fn extract_data_properties(&self, value_map: &gremlin_client::GValue) -> Result<HashMap<String, serde_json::Value>> {
    ...
    pub async fn find_paths(&self, source_id: &Uuid, target_id: &Uuid, max_depth: i32) -> Result<Vec<Vec<(GraphNode, Option<GraphRelationship>)>>> {
