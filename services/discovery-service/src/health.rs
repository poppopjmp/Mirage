...
use crate::models::{HealthCheckResult, ServiceInstance, ServiceStatus};
...
impl HealthService {
    ...
    pub async fn check_service_health(
        ...
        let status_changed = if current_state.consecutive_failures >= self.config.failure_threshold
        {
            if instance.status != ServiceStatus::Down {
                self.repo
                    .update_service_status(&instance.id, ServiceStatus::Down)
                    .await?;
                true
            } else {
                false
            ...
            if instance.status != ServiceStatus::Up {
                self.repo
                    .update_service_status(&instance.id, ServiceStatus::Up)
                    .await?;
                true
            } else {
                false
        ...
        if status_changed {
            info!(
                ...
                instance.name, current_state.status, response_time_ms
            );
        }
    ...
    pub async fn get_service_health(
        &self,
        service_id: &str,
    ) -> DiscoveryResult<Option<HealthCheckResult>> {
        let states = self.health_states.lock().await;
        ...
        match health_service.check_all_services().await {
            Ok(results) => {
                ...
                let unknown_count = results
                    .iter()
                    .filter(|r| r.status == ServiceStatus::Unknown)
                    .count();
