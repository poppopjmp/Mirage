...
use crate::models::{
    ServiceRegistrationRequest, ServiceStatus, ServiceHeartbeatRequest, ServiceQuery
};
...
async fn register_service(
    ...
    let result = service.register_service(request.into_inner()).await
        .map_err(|e| {
            match e {
                _ => {
...
async fn get_service(
    ...
    let instance = service.get_service(&id).await
        .map_err(|e| {
            match e {
                _ => {
...
async fn get_services(
    ...
    let services = service.get_all_services().await
        .map_err(|e| {
...
async fn query_services(
    ...
    let services = service.query_services(query.into_inner()).await
        .map_err(|e| {
...
async fn get_service_instances(
    ...
    let instances = service.get_service_instances(&name).await
        .map_err(|e| {
            match e {
                _ => {
...
async fn deregister_service(
    ...
    service.deregister_service(&id).await
        .map_err(|e| {
            match e {
                _ => {
...
async fn get_service_health(
    ...
    let result = health_service.get_service_health(&id).await
        .map_err(|e| {
...
async fn get_health(
    ...
    let results = health_service.get_health_results().await
        .map_err(|e| {
    ...
    let up = health_results.iter().filter(|r| r.status == ServiceStatus::Up).count();
    let down = health_results.iter().filter(|r| r.status == ServiceStatus::Down).count();
