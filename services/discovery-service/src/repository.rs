use crate::models::{ServiceInstance, ServiceStatus, ServiceQuery};
use crate::config::RedisConfig;
use crate::error::{DiscoveryError, DiscoveryResult};
use redis::{Client as RedisClient, AsyncCommands, RedisResult, FromRedisValue};
use std::sync::Arc;
use chrono::Utc;
...
impl ServiceRepository {
    ...
    pub async fn register_service(&self, instance: &ServiceInstance) -> DiscoveryResult<()> {
        ...
        let instance_json = serde_json::to_string(instance)
            .map_err(|e| DiscoveryError::Internal(format!("Failed to serialize instance: {}", e)))?;
        
        // Store the instance with an expiration (TTL)
        conn.set_ex(instance_key.clone(), instance_json, self.config.service_ttl_seconds as usize).await?;
        ...
        conn.sadd(format!("{}:services", self.config.key_prefix), &instance.name).await?;
    ...
    pub async fn update_service(&self, instance: &ServiceInstance) -> DiscoveryResult<()> {
        ...
        if !exists {
            return Err(DiscoveryError::NotFound(format!(
                "Service instance {} not found", instance.id
            )));
        }
        ...
        conn.set_ex(instance_key, instance_json, self.config.service_ttl_seconds as usize).await?;
    ...
    pub async fn deregister_service(&self, instance_id: &str) -> DiscoveryResult<()> {
        ...
        match instance_json {
            Some(json) => {
                ...
                let instance: ServiceInstance = serde_json::from_str(&json)
                    .map_err(|e| DiscoveryError::Internal(format!("Failed to deserialize instance: {}", e)))?;
                ...
                if count == 0 {
                    ...
                    conn.srem(format!("{}:services", self.config.key_prefix), instance.name).await?;
                    // Remove empty set
    ...
    pub async fn get_service_by_id(&self, instance_id: &str) -> DiscoveryResult<Option<ServiceInstance>> {
        let mut conn = self.client.get_async_connection().await?;
    ...
    pub async fn get_service_instances(&self, service_name: &str) -> DiscoveryResult<Vec<ServiceInstance>> {
        let mut conn = self.client.get_async_connection().await?;
    ...
    pub async fn update_service_status(&self, instance_id: &str, status: ServiceStatus) -> DiscoveryResult<()> {
        let instance = match self.get_service_by_id(instance_id).await? {
            ...
            None => return Err(DiscoveryError::NotFound(format!(
                "Service instance {} not found", instance_id
            ))),
        };
    ...
    pub async fn query_services(&self, query: &ServiceQuery) -> DiscoveryResult<Vec<ServiceInstance>> {
        ...
        // Filter by status
        let filtered = all_instances.into_iter().filter(|instance| {
    ...
    pub async fn cleanup_expired_services(&self) -> DiscoveryResult<usize> {
        ...
        for name in service_names {
            ...
            if count == 0 {
                // Remove service from list of known services
                conn.srem(format!("{}:services", self.config.key_prefix), &name).await?;
                // Remove empty set
                conn.del(&service_key).await?;
            }
        }
