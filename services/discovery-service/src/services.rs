use crate::models::{ServiceInstance, ServiceRegistrationRequest, ServiceResponse, ServiceStatus, ServiceQuery, ServiceHeartbeatRequest, ServiceRegistry};
...
impl DiscoveryService {
    ...
    pub async fn register_service(&self, request: ServiceRegistrationRequest) -> DiscoveryResult<ServiceResponse> {
        // Validate request
        if request.name.is_empty() {
            return Err(DiscoveryError::Validation("Service name cannot be empty".into()));
        }
        
        if request.address.is_empty() {
            return Err(DiscoveryError::Validation("Service address cannot be empty".into()));
        }
        
        if request.port == 0 {
            return Err(DiscoveryError::Validation("Service port cannot be zero".into()));
        }
    ...
    pub async fn heartbeat(&self, request: ServiceHeartbeatRequest) -> DiscoveryResult<ServiceResponse> {
        ...
        let mut instance = match self.repo.get_service_by_id(&request.id).await? {
            ...
            None => return Err(DiscoveryError::NotFound(format!(
                "Service instance {} not found", request.id
            ))),
        };
    ...
    pub async fn get_service_instances(&self, service_name: &str) -> DiscoveryResult<Vec<ServiceResponse>> {
        let instances = self.repo.get_service_instances(service_name).await?;
        
        if instances.is_empty() {
            return Err(DiscoveryError::NotFound(format!(
                "No instances found for service {}", service_name
            )));
        }
    ...
    pub async fn get_all_services(&self) -> DiscoveryResult<ServiceRegistry> {
        ...
        let responses: Vec<ServiceResponse> = instances.into_iter().map(ServiceResponse::from).collect();
