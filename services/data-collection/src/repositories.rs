...
impl JobRepository {
    ...
    pub async fn update_job_completed(&self, job_id: &Uuid, result: Option<serde_json::Value>) -> Result<()> {
        let collection = self.db.collection::<Document>("collection_jobs");
    ...
    pub async fn get_pending_jobs(&self, limit: i64) -> Result<Vec<CollectionJob>> {
        ...
        let cursor = collection
            .find(doc! {"status": CollectionStatus::Pending.to_string()}, options)
            .await
...
use crate::models::{CollectionTask, TaskStatus, TaskType, ResultSummary, CollectionTarget, TaskResult, Entity, Relationship};
use mirage_common::{Error, Result};
use mongodb::{Database, Collection, bson::{self, doc, Document, oid::ObjectId}};
...
impl TaskRepository {
    ...
    pub async fn create_task(&self, task: &CollectionTask) -> Result<Uuid> {
        ...
        let result = self.collection.insert_one(task_doc, None).await
    ...
    pub async fn update_task_status(
        ...
        if let Some(started) = started_at {
            update_doc.get_document_mut("$set").unwrap().insert("started_at", bson::to_bson(&started).unwrap());
        }
        
        if let Some(completed) = completed_at {
            update_doc.get_document_mut("$set").unwrap().insert("completed_at", bson::to_bson(&completed).unwrap());
        }
        ...
        if let Some(summary) = result_summary {
            update_doc.get_document_mut("$set").unwrap().insert("result_summary", bson::to_bson(&summary).unwrap());
        }
        ...
        let result = self.collection.find_one_and_update(filter, update_doc, options).await
...
impl ResultRepository {
    ...
    pub async fn save_result(&self, result: &TaskResult) -> Result<Uuid> {
        ...
        let doc = match bson_value {
            ...
            _ => return Err(Error::Internal("Failed to convert result to document".into())),
        };
