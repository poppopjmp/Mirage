...
impl TemplateRegistry {
    pub fn new(config: &TemplateConfig) -> Self {
        ...
        handlebars.register_template_string("new_entity_email", include_str!("../templates/new_entity_email.hbs"))
            .expect("Failed to register new_entity_email template");
        handlebars.register_template_string("new_relationship_email", include_str!("../templates/new_relationship_email.hbs"))
            .expect("Failed to register new_relationship_email template");
        handlebars.register_template_string("scan_complete_email", include_str!("../templates/scan_complete_email.hbs"))
            .expect("Failed to register scan_complete_email template");
        handlebars.register_template_string("alert_triggered_email", include_str!("../templates/alert_triggered_email.hbs"))
            .expect("Failed to register alert_triggered_email template");
        handlebars.register_template_string("system_alert_email", include_str!("../templates/system_alert_email.hbs"))
            .expect("Failed to register system_alert_email template");
        
        // Also register subject templates
        handlebars.register_template_string("new_entity_subject", "New Entity Detected: {{entity_type}} - {{value}}")
            .expect("Failed to register new_entity_subject template");
        handlebars.register_template_string("new_relationship_subject", "New Relationship Detected: {{relationship_type}}")
            .expect("Failed to register new_relationship_subject template");
        handlebars.register_template_string("scan_complete_subject", "Scan Completed: {{scan_name}}")
            .expect("Failed to register scan_complete_subject template");
        handlebars.register_template_string("alert_triggered_subject", "Alert Triggered: {{alert_name}}")
            .expect("Failed to register alert_triggered_subject template");
        handlebars.register_template_string("system_alert_subject", "System Alert: {{alert_type}}")
    ...
    pub fn render_subject(&self, notification_type: &NotificationType, data: &serde_json::Value) -> Result<String> {
        ...
        let rendered = self.handlebars.render(template_name, data)
            .map_err(|e| Error::Internal(format!("Failed to render subject template {}: {}", template_name, e)))?;
            
        Ok(rendered)
    }
    
    pub fn render_content(&self, notification_type: &NotificationType, data: &serde_json::Value) -> Result<String> {
        let template_name = match notification_type {
        ...
        let rendered = self.handlebars.render(template_name, data)
            .map_err(|e| Error::Internal(format!("Failed to render content template {}: {}", template_name, e)))?;
