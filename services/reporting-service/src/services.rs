...
use crate::models::{Report, ReportRequest, ReportFormat, ReportType, EntityData, RelationshipData, ReportTemplateContext, VisualizationData, ReportTemplate};
use crate::formatters::{HtmlFormatter, PdfFormatter, MarkdownFormatter, JsonFormatter, CsvFormatter, ExcelFormatter, ReportFormatter};
...
impl ReportService {
    pub fn new(client: Client, config: AppConfig) -> Self {
        ...
        if template_dir.exists() && template_dir.is_dir() {
                        ...
                        if file_type.is_file() {
                            if let Some(file_name) = entry.file_name().to_str() {
                                if file_name.ends_with(".hbs") {
                                    ...
                                    if let Err(e) = handlebars.register_template_file(template_name, &template_path) {
                                        tracing::error!("Failed to register template '{}': {}", template_name, e);
                                    } else {
            ...
            tracing::warn!("Template directory does not exist: {}", config.report.template_dir);
        }
        
        // Register default templates from memory as a fallback
        handlebars.register_template_string("summary_html", include_str!("../templates/summary_html.hbs"))
            .expect("Failed to register default summary HTML template");
        handlebars.register_template_string("detailed_html", include_str!("../templates/detailed_html.hbs"))
    ...
    pub async fn generate_report(&self, request: ReportRequest) -> Result<Report, mirage_common::Error> {
        // Validate request
        if request.entity_ids.is_empty() {
            return Err(Error::Validation("At least one entity ID must be provided".into()));
        }
        ...
        let formatter: Box<dyn ReportFormatter> = match request.format {
            ...
            ReportFormat::Text => return Err(Error::Validation("Text format not yet implemented".into())),
        };
    ...
    pub fn get_report_file(&self, report_id: &Uuid) -> Result<PathBuf, mirage_common::Error> {
        ...
        Err(Error::NotFound(format!("Report with ID {} not found", report_id)))
    }
    
    // Helper methods
    
    async fn fetch_entities_data(&self, entity_ids: &[Uuid]) -> Result<Vec<EntityData>, mirage_common::Error> {
        ...
        for id in entity_ids {
            ...
            if !response.status().is_success() {
                tracing::warn!("Failed to fetch entity data for ID {}: {}", id, response.status());
                continue;
            }
            
            let entity_data: serde_json::Value = response.json()
                .await
                .map_err(|e| Error::ExternalApi(format!("Failed to parse entity data: {}", e)))?;
                
            // Fetch relationships
            let relationships_url = format!("{}/api/v1/data/relationships/{}", self.config.data_storage.url, id);
            
            let relationships_response = self.client.get(&relationships_url)
                .send()
                .await
                .map_err(|e| Error::ExternalApi(format!("Failed to fetch relationships: {}", e)))?;
                
            let relationships: Vec<RelationshipData> = if relationships_response.status().is_success() {
                relationships_response.json()
                    .await
                    .map_err(|e| Error::ExternalApi(format!("Failed to parse relationships: {}", e)))?
            } else {
                Vec::new()
            };
            ...
            if let (Some(entity_id), Some(entity_type), Some(value)) = (
                entity_data["id"].as_str().map(|s| Uuid::parse_str(s).ok()).flatten(),
                ...
                entities.push(EntityData {
                    ...
                    data: entity_data.get("data").unwrap_or(&serde_json::Value::Null).clone(),
                    metadata,
                    created_at,
                    relationships,
                });
    ...
    async fn generate_visualizations(&self, entities: &[EntityData], request: &ReportRequest) -> Result<Vec<VisualizationData>, mirage_common::Error> {
        ...
        if entities.len() > 1 {
            ...
            let viz_url = format!("{}/api/v1/visualizations/graph", self.config.visualization.url);
            ...
            let response = self.client.post(&viz_url)
                .json(&viz_request)
                .send()
                .await
                .map_err(|e| Error::ExternalApi(format!("Failed to generate graph visualization: {}", e)))?;
                
            if response.status().is_success() {
                let viz_result: serde_json::Value = response.json()
                    .await
                    .map_err(|e| Error::ExternalApi(format!("Failed to parse visualization result: {}", e)))?;
                    
                if let (Some(viz_id), Some(data)) = (
                    viz_result["id"].as_str().map(|s| Uuid::parse_str(s).ok()).flatten(),
                    ...
                    visualizations.push(VisualizationData {
                        ...
                        description: Some(format!("Relationships between {} entities", entity_ids.len())),
                    });
                }
            } else {
                tracing::warn!("Failed to generate graph visualization: {}", response.status());
            }
        }
        
        // If appropriate for the report type, add entity type distribution chart
        if matches!(request.report_type, ReportType::Summary | ReportType::Executive | ReportType::Detailed) {
            let viz_url = format!("{}/api/v1/visualizations/chart", self.config.visualization.url);
            ...
            let response = self.client.post(&viz_url)
                ...
                .send()
                .await
                .map_err(|e| Error::ExternalApi(format!("Failed to generate chart visualization: {}", e)))?;
            ...
            if response.status().is_success() {
                ...
                if let (Some(viz_id), Some(_data)) = (
                    ...
                    visualizations.push(VisualizationData {
                        ...
                        description: Some("Distribution of entity types in the dataset".to_string()),
                    });
                }
            } else {
                tracing::warn!("Failed to generate pie chart visualization: {}", response.status());
            }
        }
