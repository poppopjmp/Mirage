...
use crate::models::{CollectionTask, TaskStatus, TaskResult, ResultSummary};
...
pub async fn start_worker_pool(
    ...
    tracing::info!("Starting worker pool with min={}, max={} workers", min_workers, max_workers);
    
    // Create a channel for workers to report task completion
    let (completion_tx, mut completion_rx) = mpsc::channel::<(Uuid, bool, Option<String>, Option<TaskResult>)>(100);
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            if active_count < min_workers && queue_size > 0 {
                tracing::info!("Starting additional workers: active={}, min={}", active_count, min_workers);
        ...
        while let Some((task_id, success, error_message, task_result)) = completion_rx.recv().await {
            ...
            let result_summary = if let Some(result) = &task_result {
                Some(ResultSummary {
                    ...
                    data_size_bytes: serde_json::to_string(&result).unwrap_or_default().len() as u64,
            ...
            if let Err(e) = completion_task_repo.update_task_status(
                &task_id,
                status,
                None,
                Some(Utc::now()),
    ...
    tokio::spawn(async move {
        loop {
            ...
            if permits_available > 0 && active_count < max_workers {
                ...
                let task = match processing_task_repo.get_task_by_id(&queue_task.task_id).await {
                    Ok(Some(task)) => task,
                    Ok(None) => {
                        tracing::error!("Task {} from queue not found in database", queue_task.task_id);
                        continue;
                    }
                    Err(e) => {
                ...
                if let Ok(permit) = processing_semaphore.clone().try_acquire_owned() {
                    ...
                    tokio::spawn(async move {
                        ...
                        if let Err(e) = worker_task_repo.update_task_status(
                            &task.id,
                            TaskStatus::Running,
                            Some(Utc::now()),
                            None,
                        ...
                        if let Err(e) = worker_completion_tx.send((task.id, success, error, result)).await {
            ...
            match task_queue.dequeue_task().await {
                Ok(Some(queued_task)) => {
                    ...
                    match task_repo.get_task_by_id(&queued_task.task_id).await {
                        Ok(Some(task)) => task,
                        Ok(None) => {
                            tracing::error!("Task ID {} from queue not found in database", queued_task.task_id);
                            continue;
                        },
                        Err(e) => {
